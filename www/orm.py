...
async def create_pool(loop, **kw):
    logging.info('createing database connection pool...')
    global __pool
    __pool = await aiomysql.create_pool(minsize=kw.get('minsize', 1),
...
async def select(sql, args, size=None):
    ...
    async with __pool.get() as conn:
        ...
        logging.info('row returned: %s' % rs)
    return rs
...
class ModelMetaClass(type):
    def __new__(cls, name, bases, attrs):
        ...
        attrs['__primery_key__'] = primaryKey
